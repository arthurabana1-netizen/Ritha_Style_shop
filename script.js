...
async function loadProducts() {
    ...
    try {
        ...
        if (products.length > 0) {
            ...
            checkForSharedProduct(); // Check URL on load
        } else {
...
// 5. Sharing Logic
function getShareLink(productName) {
    const encodedName = encodeURIComponent(productName);
    // Use the base URL (without any existing parameters)
    const baseUrl = window.location.origin + window.location.pathname.replace(/\/$/, '');
    return `${baseUrl}?id=${encodedName}`;
}

function checkForSharedProduct() {
    const params = new URLSearchParams(window.location.search);
    const sharedProductId = params.get('id');

    if (sharedProductId) {
        const decodedId = decodeURIComponent(sharedProductId);
        const productToOpen = products.find(p => p.name === decodedId);
        
        if (productToOpen) {
            openPreview(productToOpen);
        }
    }
}
...
function openPreview(p) {
    ...
    const shareLink = getShareLink(p.name);
    ...
    document.querySelector('#preview-modal .modal-content').innerHTML = `
        ...
        <div id="preview-details">
            ...
            <div class="share-section" style="text-align:center; margin: 15px 0; border-top: 1px solid #eee; padding-top: 10px;">
                <button onclick="copyToClipboard('${shareLink}')" class="share-btn">
                    <i class="fa-solid fa-share-alt"></i> Share This Product Link
                </button>
                <p id="copy-message" style="margin-top:5px; font-size:0.8rem; color:var(--secondary-color); visibility:hidden;">Link copied to clipboard!</p>
            </div>
    ...
    // Update the URL to the share link without reloading the page
    history.pushState(null, '', shareLink);
}
...
function closePreviewModal() {
    ...
    // Reset URL to the base path when closing the modal
